...
export const initCommand = new Command('init')
  ...
  .action(async (projectName, options) => {
    ...
    const { uid, projtype } = await inquirer.prompt([{
      ...
      message: '请选择工程类型',
      choices: [
        { name: "Worker", value: "worker", description: "Worker后端项目，适合复杂业务" },
        { name: "Vue", value: "vue", description: "Vue项目，前后端工程" },
        { name: "Simple", value: "simple", description: "简单Worker项目，不包含数据库" },
      ],
      default: 'worker'
    },])
    ...
    switch (projtype) {
      ...
      case "simple": {
        await createWorkerProject(projectName, jsonData, "https://git.cloud.zhishudali.ink/template/vv-template-simple.git");
        break;
      }
    }
  });
