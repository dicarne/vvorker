...
async function checkDeploymentResult(apiClient: any, uid: string, taskId: string) {
  ...
  console.log(pc.yellow("等待部署和迁移完成..."));
  ...
  while (Date.now() - startTime < maxWaitTime) {
    try {
      // 检查任务状态
      const taskResp = await apiClient.post(`/api/ext/task/check-interrupt-task`, {
        worker_uid: uid,
        trace_id: taskId
      });

      if (taskResp.data.code === 0) {
        const status = taskResp.data.data.status;
        ...
        if (status === 'failed' || status === 'canceled') {
          // 获取任务详细信息（包括错误信息）
          console.log(pc.red(taskResp.data.data.result));
          return;
        }
      }
    } catch (error) {
  ...
  // 超时后提示用户
  console.log(pc.yellow("部署超时，请稍后检查部署状态"));
  console.log(pc.gray(`任务ID: ${taskId}`));
}
