...
type InternalWhiteListCreateRequest struct {
	...
	AllowWorkerUID string `json:"allow_worker_uid"`
...
func CreateInternalWhiteListEndpoint(c *gin.Context) {
	...
	whiteList := models.InternalServerWhiteList{
		...
		AllowWorkerUID: request.AllowWorkerUID,
...
func ListInternalWhiteListEndpoint(c *gin.Context) {
	...
	var whiteLists []models.InternalServerWhiteList
	if err := db.Where(&models.InternalServerWhiteList{WorkerUID: request.WorkerUID}).Offset((request.Page - 1) * request.PageSize).Limit(request.PageSize).Find(&whiteLists).Error; err != nil {
		common.RespErr(c, common.RespCodeInternalError, err.Error(), nil)
		return
	}
	common.RespOK(c, common.RespMsgOK, gin.H{
...
func DeleteInternalWhiteListEndpoint(c *gin.Context) {
	...
	if err := db.Where(&models.InternalServerWhiteList{WorkerUID: request.WorkerUID, AllowWorkerUID: request.AllowWorkerUID}).Delete(&models.InternalServerWhiteList{}).Error; err != nil {
		common.RespErr(c, common.RespCodeInternalError, err.Error(), nil)
		return
	}
	common.RespOK(c, common.RespMsgOK, nil)
